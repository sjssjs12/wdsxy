	...
	_createClass(LunarHelp, [
		...
		},
		{
			...
			value: function getLunarDayName() {
				...
				// console.log( '阴历'+this.cDay(this.day));
	...
	_createClass(SimpleCalendar, [
		...
		},
		{
			...
			value: function create() {
				...
				title.innerHTML =
					title.innerHTML + '<div class="sc-actions"><span class="sc-return-today ">今天</span></div>';
				for (var i = 0; i < 7; i++) {
				...
				for (var i = 0; i < 42; i++) {
					days.innerHTML =
						days.innerHTML +
						'<div class="sc-item"><p class="mark sc-mark-hide">•</p><div class="day"></div><div class="lunar-day"></div></div>';
				}
				//添加下拉框数据
		...
		},
		{
			...
			value: function updateEvent() {
				...
				returntoday.onclick = function() {
					...
					//console.log('-----------' + selectYear);
					selectYear.value = selectYear.innerText;
					selectMonth.value = calendar.tmonth;
					//console.log(selectMonth.value);
					//console.log('-----------' + selectMonth);
					$('.sc-select-year').text(calendar.tyear);
					var tn = SimpleCalendar.prototype.languageData.months_CH[selectMonth.value - 1];

					$('.sc-select-month').text(tn);
					//$('.sc-select-month').text(selectMonth.value);

					console.log($('.sc-select-month').text());
					calendar.update();
